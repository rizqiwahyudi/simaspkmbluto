...
include('../../../sw-library/sw-function.php');
    ...
    while ($aRow = mysqli_fetch_array($rResult)){$no++;
      extract($aRow);
        ...
        $bln = date("m");
      $query_hadir="SELECT apel_id FROM apel WHERE MONTH(apel_date)='$bln' AND employees_id=$aRow[id]";
      $hadir= $connection->query($query_hadir);
        ...
        for ($i=1 ; $i<count($aColumns) ; $i++){
            ...
            $row[] = '<div class="text-center">
                       <a href="./apel&op=views&id='.epm_encode($aRow['id']).'" class="btn btn-warning btn-sm enable-tooltip" title="Detail"><i class="fa fa-eye" aria-hidden="true"></i> Detail</a>
                      </div>';
        }
        $output['aaData'][] = $row;
        $no++;
